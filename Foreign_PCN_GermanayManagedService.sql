SELECT
  ...
  ARRAY_AGG(
         CASE 
         WHEN cc.parkingSession.vehicleDetails.state not in ('D','DE') then 1
         ELSE NULL END 
...
FROM `sc-neptune-production.contravention_parking_cases_v2.contravention_parking_cases_v2_spGermanyManagedService` cc
...
WHERE
  STRUCT(cc.id, CAST(cc.rev as INT64)) IN 
         (
            ...
            FROM `sc-neptune-production.contravention_parking_cases_v2.contravention_parking_cases_v2_spGermanyManagedService`
 ...
 And cc.parkingSession.vehicleDetails.state not in ('D','DE') 
group by all